...
fn parse(code: []const u8) !std.ArrayList(Op) {
    ...
    for (code) |char| {
        const op_opt: ?Op = switch (char) {
            ...
            '>' => Op.right,
            '[' => blk: {
                try jump_stack.append(@intCast(ops.items.len));
                break :blk Op{ .jumpIfZero = 0xDEADBEEF };
            },
            ']' => blk: {
                const pair_index = jump_stack.pop();
                ops.items[pair_index] = Op{ .jumpIfZero = @intCast(ops.items.len) };
                break :blk Op{ .jumpBackIfNonZero = pair_index };
            },
            '.' => Op.print,
            else => null,
        };
